...
style="background-image: url('{{ asset('images/img/textura3.webp') }}');">
    ...
    <section class="flex flex-col md:flex-row md:gap-10 w-full px-[5%] font- py-16">
        ...
        <div class="w-full md:w-3/12 gap-5  grid grid-cols-1">
            {{-- <h3 class="font-jakartaMedium text-base text-[#333] font-semibold">Buscar post</h3>
            ...
            <div class="md:basis-1/6 flex flex-col gap-5" >
                <h3 class="font-jakartaMedium text-base font-semibold text-[#333]">Blog categorias</h3>
                <div class="flex flex-col gap-3 font-jakartaRegular">
                    <a href="{{ route('blog', 0) }}"
                        class="text-sm py-3 px-4 rounded-lg font-semibold tracking-wide {{ $filtro == 0 ? 'bg-primario text-white' : 'text-[#333] bg-[#E6E4E5] bg-opacity-40 ' }} ">Todas</a>
                        @foreach ($categorias as $item)
                        <a href="{{ route('blog', $item->id) }}"
                            class="text-sm py-3 px-4 rounded-lg font-normal
                                {{ $item->id == $filtro ? 'bg-primario font-semibold text-white' : 'text-[#333] bg-[#E6E4E5] bg-opacity-40' }}">
                            {{ $item->name }}
                        </a>
                        @endforeach
                </div>

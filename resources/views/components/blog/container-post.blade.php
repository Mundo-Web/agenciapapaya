 ...
 <div class="flex flex-col gap-3 font">
     <div class="flex flex-col gap-4">
         <div class="relative flex justify-center items-center">
             <a href="{{ route('detalleBlog', $post->id) }}" class="w-full">
                 <img src="{{ asset($post->url_image . $post->name_image) }}"
                     class="w-full object-cover h-56 sm:h-64 md:h-64 rounded-2xl" alt="blog"></a>
             <div class="absolute top-0 left-0 pt-4 pl-4">
                 <h3
                     class="text-sm md:text-sm font-jakartaMedium bg-primario text-white px-3 pt-1 pb-2 rounded-3xl">
                     {{ $category->name }}</h3>
             </div>
         </div>
         ...
         <div class="flex justify-start items-center gap-2">
             ...
             <p class="text-[#444444] font-jakartaMedium font-normal text-xs">Publicado
         ...
         <div class="flex flex-col justify-start items-start gap-1 -mt-2 md:-mt-3">
             <a href="{{ route('detalleBlog', $post->id) }}">
                 <h2 class="text-lg font-jakartaMedium font-normal text-[#333333]">{{ $post->title }}
                 </h2>
             </a>

             <a  href="{{ route('detalleBlog', $post->id) }}" 
                 class="text-sm font-jakartaMedium font-bold text-primario  leading-tight flex flex-row gap-2 items-center">
                 Leer más
